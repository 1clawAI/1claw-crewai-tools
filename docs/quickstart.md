...
5. An **LLM API key** for CrewAI — OpenAI (`OPENAI_API_KEY`) or Google Gemini (`GOOGLE_API_KEY`).
...
For Google Gemini support, also install the provider extra:
...
| `OPENAI_API_KEY` | OpenAI key (default LLM provider for CrewAI). |
| `GOOGLE_API_KEY` | Google Gemini key (alternative LLM provider). |
...
agent = Agent(
    ...
    backstory="You call the oneclaw_vault tool for secrets.",
    tools=[tool],
    verbose=True,
)
...
llm = LLM(model="gemini/gemini-2.0-flash", api_key=os.environ["GOOGLE_API_KEY"])
...
- **CrewAI verbose mode prints secrets** — when `verbose=True`, CrewAI's executor logs the raw return value of every tool call to stdout. Use `verbose=False` in production or redirect stdout.
...
See `examples/test_live.py` — runs three phases (raw client, single-agent crew, two-agent crew) against the real 1Claw API. Requires `ONECLAW_*` env vars and an LLM key.
